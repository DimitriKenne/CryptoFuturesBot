...
def generate_feature_names(config: FeatureConfig = None) -> List[str]:
    ...
    # --- 1. Price Transformations (from PriceFeatureCalculator) ---
    feature_names.extend(['log_returns', 'typical_price', 'mid_price', 'body_range', 'open_close_diff', 'high_low_diff'])

    # --- 2. Momentum Indicators (from IndicatorFeatureProcessor.add_core_technical_indicators) ---
    for period in config.rsi_periods:
        feature_names.append(f'rsi_{period}')
    
    stoch_d_period = 3 # Hardcoded in IndicatorFeatureProcessor, so consistent here
    ...
    for period in config.mfi_periods: # MFI is also calculated here
        feature_names.append(f'mfi_{period}')


    # --- 3. Trend Indicators (from IndicatorFeatureProcessor.add_core_technical_indicators) ---
    ...
    # --- 4. Volatility Indicators (from IndicatorFeatureProcessor.add_core_technical_indicators) ---
    ...
    # --- 5. Volume Indicators (from IndicatorFeatureProcessor.add_core_technical_indicators) ---
    feature_names.append('obv')
    for period in config.volume_periods:
        feature_names.append(f'cmf_{period}')
        # MFI is included above, so no need to duplicate here if it's the same
        # feature_names.append(f'mfi_{period}') 
    feature_names.append('volume_osc') # Volume Oscillator

    # --- 6. Statistical Features (from IndicatorFeatureProcessor.add_core_technical_indicators) ---
    ...
    # --- 7. Custom Pattern and FVG Features (from PriceActionFeatureProcessor.add_custom_pattern_features) ---
    ...
    # --- 8. Pivot Point Features (from PriceActionFeatureProcessor.add_pivot_point_features) ---
    ...
    # --- 9. Support/Resistance Features (from PriceActionFeatureProcessor.add_support_resistance_features) ---
    ...
    # --- 10. Derived Features (from IndicatorFeatureProcessor.add_derived_features) ---
    ...
    else: # If ATR not available, these are still generated but not normalized
        for period in config.support_resistance_periods:
            # These are already added in section 9.
            # feature_names.append(f'resistance_{period}') 
            # feature_names.append(f'support_{period}')
            feature_names.append(f'dist_to_support_{period}')
            feature_names.append(f'dist_to_resistance_{period}')
    ...
    # --- 11. Breakout Features (from PriceActionFeatureProcessor.add_breakout_features) ---
    feature_names.extend([
        'is_support_broken_strong_vol', 'is_resistance_broken_strong_vol',
        'is_bull_wick_at_resistance', 'is_bear_wick_at_support'
    ])

    # --- 12. Lagged Features (added by FeatureEngineer._add_lagged_features) ---
    ...
    # --- 13. Differenced Features (added by FeatureEngineer._add_differenced_features) ---
    ...
    # Filter out duplicates (if any) and return sorted
