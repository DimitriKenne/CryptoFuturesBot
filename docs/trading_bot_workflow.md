...
6.  **Critical - The Reconciliation Loop:** This is the core of a safe startup.
    ...
    *   **Phase A: Fetch Ground Truth**
        ...
        3.  **Filter by Tag:** The bot immediately filters the list of open orders, keeping only those containing its unique `bot_id`. **Untagged orders are ignored.**

    *   **Phase B: Reconcile State & Enforce Protection**
        1.  **Reconcile Active Position:**
            *   **Match Found:** If the exchange has one position that matches the bot's loaded position, it is adopted.
        ...
        2.  **Reconcile SL/TP Orders:**
            *   Based on the now-synced active position, the bot knows what its ideal SL/TP orders should be.
            *   **Missing Protection:** If an ideal SL or TP order is *missing* from the exchange, it is placed immediately with the correct `bot_id` tag.
            *   **Orphan Orders:** Any **bot-tagged** order on the exchange that is *not* the reconciled position's verified SL or TP order is considered an orphan and is cancelled.

7.  **Launch Main Loop:** Only after the state is fully reconciled and the position (if any) is confirmed to be protected by tagged SL/TP orders does the main trading loop begin.
...
3.  **Execute & Verify Entry:** The `TradeCycleProcessor` executes the robust entry sequence, ensuring every order placed (entry, SL, TP) is given a unique `newClientOrderId` containing the `bot_id`.
